...
public class Utils {
    ...
    public static boolean withinGrid(Point pos, int height, int width) {
    ...
    public static class Graph<T>{
        ...
        public Edge<T> addEdge(T source, T target, int cost){
            var v1 = Vertice.of(source);
            var v2 = Vertice.of(target);
            ...
            Edge<T> edge = Edge.of(Vertice.of(source),Vertice.of(target),cost);
            if(edges.add(edge)){
                return edge;
            }
        ...
        public int getEdgeCost(Vertice<T> source, Vertice<T> target){
            for(var edge : edges){
                if(edge._from.equals(source) && edge._to.equals(target)){
                    return edge._cost;
                }
            }
        ...
        public Edge<T> getEdge(Vertice<T> source, Vertice<T> target){
            for(var edge : edges){
                if(edge._from.equals(source) && edge._to.equals(target)){
                    return edge;
                }
    ...
    public static class Edge<V> {
        ...
        Edge(Vertice<V> from, Vertice<V> to, int cost){
            _from = from;
            _to = to;
            _cost = cost;
            _hash = Objects.hash(from,to,cost); 
        }

        static <V> Edge<V> of(Vertice<V> from, Vertice<V> to, int cost){
            return new Edge<>(from,to,cost);
        }

        @Override
        public String toString() {
            return _from + " --" + _cost + "--> " + _to;
        }
    ...
    public static class Vertice<ID>{
        public ID _identifier;
        int hash;
        ...
        public static <ID> Vertice<ID> of(ID id){
        ...
        public boolean equals(Object obj) {
            ...
            if(obj instanceof Vertice v){
                return v._identifier.equals(_identifier);
            }
